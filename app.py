...
def voice():
    ...
    gather.say("Hi, thanks for calling Smart Appliances Services. I'll take your details and have someone call you back shortly. Can I get your name please?", voice="Polly.Olivia-Neural")
    resp.append(gather)
...
def got_name():
    ...
    gather.say(f"Thanks {session['name']}. And what suburb are you in?", voice="Polly.Olivia-Neural")
    resp.append(gather)
    return Response(str(resp), mimetype="text/xml")
...
def got_suburb():
    ...
    gather.say("Great. And briefly, what's the issue with your appliance?", voice="Polly.Olivia-Neural")
    resp.append(gather)
    return Response(str(resp), mimetype="text/xml")
...
def got_issue():
    ...
    resp = VoiceResponse()
    resp.say("Perfect, thank you. We've got your details and someone will be in touch with you shortly. Have a great day!", voice="Polly.Olivia-Neural")
    return Response(str(resp), mimetype="text/xml")
...
def summarise_with_claude(session):
    message = anthropic_client.messages.create(
        ...
        max_tokens=200,
        messages=[{"role": "user", "content": f"Write a short SMS for a smart appliances business owner. Client details: Name: {session['name']}, Suburb: {session['suburb']}, Issue: {session['issue']}, Phone: {session['caller']}. Format: 📞 New call — [Name], [Suburb]. Issue: [brief issue]. Call back: [number]. Return SMS text only."}]
    )
